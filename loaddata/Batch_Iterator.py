    ...
    def convert_word2id(self, insts, operator, operator_static):
        ...
        for inst in insts:
            ...
            for index in range(inst.chars_size):
                ...
                static_charID = operator.static_char_alphabet.loadWord2idAndId2Word(char)
                # if charID is None:
                if charID == -1:
                    charID = operator.char_UnkID
                if static_charID == -1:
                    static_charID = operator.static_char_UnkID
            ...
            for index in range(inst.bichars_size):
                ...
                static_bichar_left_ID = operator.static_bichar_alphabet.loadWord2idAndId2Word(bichar_left)
                # if bichar_left_ID is None:
                if bichar_left_ID == -1:
                    bichar_left_ID = operator.bichar_UnkID
                if static_bichar_left_ID == -1:
                    static_bichar_left_ID = operator.static_bichar_UnkID
            ...
            for index in range(inst.bichars_size):
                ...
                static_bichar_right_ID = operator.static_bichar_alphabet.loadWord2idAndId2Word(bichar_right)
                # if bichar_right_ID == -1:
                # if bichar_right_ID is None:
                if bichar_right_ID == -1:
                    bichar_right_ID = operator.bichar_UnkID
                if static_bichar_right_ID == -1:
                    static_bichar_right_ID = operator.static_bichar_UnkID
        ...
        for id_inst in range(batch_length):
            ...
            for id_char_index in range(max_char_size):
                if id_char_index < inst.chars_size:
                    ...
                    batch_static_char_features.data[id_inst][id_char_index] = operator.static_char_PaddingID
            ...
            for id_bichar_right_index in range(max_bichar_size):
                if id_bichar_right_index < inst.bichars_size:
                    ...
                    batch_static_bichar_right_features.data[id_inst][id_bichar_right_index] = operator.static_bichar_PaddingID
